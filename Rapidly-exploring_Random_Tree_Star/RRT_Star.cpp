...
/**
 * @brief Construct a new rrt star::rrt star object
 *
 ...
 * @param connectCircleDist  rewire的探索半径
 * @param searchUntilMaxIter
 */
...
/**
 * @brief 计算周围一定半径内的所有邻近节点
 * @param new_node
 * @return 所有邻近节点索引
 */
vector<int> RRT_Star::findNearInds(Node *new_node)
{
...
/**
 * @brief 更新以paren_node为父节点到起点的所有cost
 *
 * @param parent_node
 */
void RRT_Star::propagateCostToLeaves(Node *parent_node)
{
 ...
 * @param near_inds
 */
void RRT_Star::rewire(Node *new_node, vector<int> near_inds)
{
    for (int i : near_inds)
    {
...
/**
 * @brief 在新产生的节点 $x_{new}$ 附近以定义的半径范围$r$内寻找所有的近邻节点 $X_{near}$，
 ...
 * @return
 */
Node *RRT_Star::chooseParent(Node *new_node, vector<int> near_inds)
{
    if (near_inds.empty())
        return nullptr;
    vector<double> costs;
