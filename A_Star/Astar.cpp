 ...
 *
 */
void Astar::setObstale(vector<double> &ox, vector<double> &oy)
{
 ...
 * @return 规划后的路径
 */
pair<vector<double>, vector<double>> Astar::planning(const vector<double> start, const vector<double> goal)
{
    ...
    while (true)
    {
        ...
        for (vector<double> move : motion)
        {
            ...
            node->cost += calHeuristic(goal_node, node);
...
/**
 * @brief 启发函数计算，与dijkstra不同
 *
 * @param n1
 * @param n2
 * @return double
 */
double Astar::calHeuristic(Node *n1, Node *n2)
{
    double w = 1.0; // 启发函数权重
 ...
 * @param oy 障碍物y
 */
void Astar::set(const vector<double> &st, const vector<double> &go, const vector<double> &ox, const vector<double> &oy)
{
    Astar::st = st;
    Astar::go = go;
    Astar::ox = ox;
