...
Node::Node(double x, double y) : x(x), y(y), parent(NULL), cost(0) {}
 ...
 */
vector<double> RRT::calDistanceAngle(Node *from_node, Node *to_node)
{
 ...
 * @return
 */
bool RRT::isInsidePlayArea(Node *node)
{
 ...
 * @return 最近的节点索引
 */
int RRT::getNearestNodeIndex(vector<Node *> node_list, Node *rnd_node)
{
    int min_index = -1;
...
/**
 * @brief 随机采样 以(100-goal_sample_rate%的概率随机生长，(goal_sample_rate)%的概率朝向目标点生长
 * @return 生成的节点
 */
Node *RRT::sampleFree()
{
...
void RRT::setBegin(Node *begin)
{
    RRT::begin = begin;
}

void RRT::setEnd(Node *end)
{
    RRT::end = end;
}
 ...
 * @return
 */
Node *RRT::steer(Node *from_node, Node *to_node, double extend_length)
{
